...
class Encoder_TRANSFORMEROLDBN(nn.Module):
    def __init__(self, modeltype, num_frames, audio_dim=1024, pos_dim=7, pose_latent_dim=64,
        ...
        self.mu_BN = nn.BatchNorm1d(self.audio_latent_dim, affine=False, eps=1e-8)
        self.sigma_BN = nn.BatchNorm1d(self.audio_latent_dim, affine=False, eps=1e-8)
        self.scaler = Scaler()
    ...
    def forward(self, batch):
        ...
        fn, b, c = mu.size()
        mu = rearrange(mu, 'fn b c -> (fn b) c')
        mu = self.mu_BN(mu)
        mu = self.scaler(mu, mode='positive')
        mu = rearrange(mu, '(fn b) c -> fn b c', fn = fn, b = b)

        logvar = rearrange(logvar, 'fn b c -> (fn b) c')
        logvar = self.sigma_BN(logvar)
        logvar = self.scaler(logvar, mode='negative')
        logvar = rearrange(logvar, '(fn b) c -> fn b c', fn = fn, b = b)
        # logvar = - torch.ones_like(logvar) * 1e10

        return {"mu": mu, "logvar": logvar}


class Decoder_TRANSFORMEROLDBN(nn.Module):
    def __init__(self, modeltype, num_frames, audio_dim=1024, pos_dim=7, pose_latent_dim=64,
