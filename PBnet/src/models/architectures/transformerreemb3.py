...
import math 
from src.models.architectures.transformerdecoder2 import TransformerDecoderLayer

# try to use RoPE correctly 
...
class Encoder_TRANSFORMERREEMB3(nn.Module):
...
class Decoder_TRANSFORMERREEMB3(nn.Module):
    def __init__(self, modeltype, num_frames, audio_dim=1024, pos_dim=7, pose_latent_dim=64,
        ...
        seqTransDecoderLayer = TransformerDecoderLayer(d_model=self.pose_latent_dim,
                                                          nhead=self.num_heads,
                                                          dim_feedforward=self.ff_size,
                                                          dropout=self.dropout,
    ...
    def forward(self, batch):
        ...
        time_rel_pos_bias = self.time_rel_pos_bias(nframes, device=x.device)

        timequeries = self.init_proj(timequeries)

        timequeries = self.init_temporal_attn(timequeries.permute(1, 0, 2), pos_bias=time_rel_pos_bias).permute(1, 0, 2)
        ...
        # num_frames, bs, 64
        output = self.seqTransDecoder(tgt=timequeries, memory=z, tgt_mask=time_rel_pos_bias.repeat(bs, 1, 1, 1),
                                      tgt_key_padding_mask=~mask)
        
        output = self.finallayer(output).permute(1, 0, 2) # .reshape(nframes, bs, self.pos_dim) # num_frames, bs, 6
