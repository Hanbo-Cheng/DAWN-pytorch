...
from extract_init_states.FaceBoxes import FaceBoxes_ONNX
...
class VideoGenerator:
    def __init__(self, args):
        ...
        # 初始化配置
        self.config_path = 'configs/mb1_120x120.yml'
        ...
        self.wav2vec2_processor = Wav2Vec2Processor.from_pretrained("./pretrain_models/hubert_ckp")
        print("Loading the HuBERT Model...")
        self.hubert_model = HubertModel.from_pretrained("./pretrain_models/hubert_ckp", from_tf=True)
        ...
        # 初始化视频生成模型为None，延迟加载
        self.video_model = self._init_video_model(self.video_config)
    ...
    def process_audio(self):
        ...
        try:
            ...
            with tempfile.NamedTemporaryFile('w', suffix='.wav', dir='./') as temp_wav:
                ...
                # 提取HuBERT特征
                hubert_hidden = self._get_hubert_from_16k_speech(speech_16k, device='cuda:1')
    ...
    def _ensure_video_model(self, model_config):
        """确保视频生成模型已经初始化。
        
        如果模型尚未初始化或配置发生变化，则重新初始化模型。
        
        Args:
            model_config (dict): 模型配置字典
        """
        if self.video_model is None:
            self.video_model = self._init_video_model(model_config)
        else:
            # 检查配置是否发生变化
            current_config = self.video_model.get_config()  # 假设模型有获取配置的方法
            if current_config != model_config:
                print("模型配置已更改，重新初始化模型...")
                self.video_model = self._init_video_model(model_config)
